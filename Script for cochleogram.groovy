/*
    *********   Count cochlea hair cells    **********

Description:
Semi-automated script to count cells along and around a drawn line in the cochlea.
Uses custom trained StarDist model to segment nuclei.

Expected input:
Single channel image stacks (or single planes), in Bio-formats readable format.
    The image file format is specified as the variable 'suffix' in this script.
A StarDist 2D trained model (zip file), with NMS and probability threshold.
    Must be specified in the script parameters (after launching the script).

Workflow:
- Image in a folder a processed one by one. If the file is a stack, it will be max-
projected automatically.
- If the experimental cochlea recognition is selected the script will try to match
a snail like line onto the cochlea. This line can be redrawn manually if not satisfying.
[Longer straight segments will result in poor approximation of distance to apex when
categorising the counting segments]
- Subsequently, you can draw straight lines along 20 inner hair cell nuclei
(multiple lines are possible)
- Each line will be processed sequentially, for counting the cells within a rectangle
around the drawn line. You can correct the nuclei recognition manually with the ROI manager,
and need to specify the correct number for inner hair cells (default is 20).
- Results are save to .csv file, which will include the number of inner, outer and total hair cells,
and also the approximate distance fo the counting segment towards the apex (0% = apex).
...
Tested with Fiji version: 2.1.0/1.53c
...
#@Boolean(label="Cochlea recognition (experimental)", value=true) cochlearRecog


// variables
suffix = ".nd2" // Fixme revert back to nd2
destination = dir.getAbsolutePath() + File.separator
result = "sep=;\nImage;# Line;Distance from apex (0% = apex);Inner hair cell count;Outer hair cell count;Total cell count\n"
// used to define the different segments of the cochlea
...
def main() {
    ...
    for (image in list) {
        ...
        // -------------     experimental cochlear recognition  --------------  //
        snailLineROI = null // initialize this variable
        if (cochlearRecog) {
        ...
        // -----------      adjust/create the cochlea outline   -----------------   //
        (impCochlea, snailLineROI) = adjustCochlea(imp, snailLineROI)
        impCochlea.show() // flattened image with cochlea outline

        //  ---------   allow drawing lines for measurements    --------------- //
        rm = RoiManager.getInstance()
        if (rm == null) rm = new RoiManager()
        else rm.reset()
        IJ.setTool("line")
        ...
        for (line in drawnLines) {
            //  ---------   Define the distance from apex    ---------------------   //
            category = calcLinePosition(line, snailLineROI)
            ...
            IJ.setTool("oval")
            baseCount = checkCellCountDialog("Adjust the cell count")
...
/**
 * Manually adjust or create a line along the cochlea
 * @param imp = original Z-projection imagePlus
 * @param snailLineROI = PolygonRoi of the automatic cochlea detection
 * @return = [flattened ImagePlus with cochlea outline, cochlea line as PolygonRoi]
 */
def adjustCochlea(ImagePlus imp, PolygonRoi snailLineROI) {
    // create the dialog
    wasLineDrawn = null // make sure a line was drawn
    continueBoolean = false // to reset overlay and draw new line
    while (wasLineDrawn == null) {
        IJ.setTool("freeline")
        drawSnailDialog = new NonBlockingGenericDialog("Draw/correct line along the cochlea")
        drawSnailDialog.addMessage("Please draw one line along the cochlea starting at the center.")
        if (snailLineROI != null) {
            drawSnailDialog.addMessage("Check the automatically detected cochlea checkbox,\n" +
                    "to accept it, or draw a new line.")
            drawSnailDialog.addCheckbox("Accept the automatically detected cochlea?", false)
        }
        drawSnailDialog.addMessage("Press OK to continue.")
        drawSnailDialog.hideCancelButton()
        drawSnailDialog.setSize(400, 300)
        drawSnailDialog.showDialog()
        continueBoolean = false // to know if points should be added to line
        if (snailLineROI != null) {
            if (!drawSnailDialog.getNextBoolean()) {
                snailLineROI = imp.getRoi() as PolygonRoi// get the manually drawn ROI
                wasLineDrawn = imp.getRoi()
                continueBoolean = true
            }
            else {
                // snailLineROI stays the same
                wasLineDrawn = true // random assignment
            }
        } else {
            snailLineROI = imp.getRoi() as PolygonRoi // get the manually drawn ROI
            wasLineDrawn = imp.getRoi()
    ...
    if (continueBoolean) {
        calcSnailLine(snailLineROI) // fills the RoiManager with points
        rm = RoiManager.getInstance()
        rm.addRoi(snailLineROI)
        // reset the overlay
        snailOL = new Overlay()
        imp.setOverlay(snailOL)
        rm.moveRoisToOverlay(imp)
        imp.setHideOverlay(false)
    }
    // create temporary image for line selection
    IJ.run("Labels...", "color=white font=12 show use draw") // show overlay labels
    impTemp = imp.flatten()
    imp.hide()
    return [impTemp, snailLineROI]
}


/**
 * Calculates the closest point of the middle of the query line
 * to the cochlea line (% from apex). It's approximate...
 * @param line = query line (type not specified, but probably Roi)
 * @param snail = PolygonRoi line of the cochlea
 * @return = % of cochlea position closest to the middle of the line of interest
 */
def calcLinePosition(Roi line, PolygonRoi snail) {
    // determine location along the snail
    lineCenterX = line.x1 + Math.abs( ( (line.x1 - line.x2) / 2) )
    lineCenterY = line.y1 + Math.abs( ( (line.y1 - line.y2) / 2) )

    xCoo = snail.getFloatPolygon().xpoints
    yCoo = snail.getFloatPolygon().ypoints
    totalLength = snail.getUncalibratedLength()
    // calculate distance to each point of the cochlea line
    distances = []
    for (int i = 0; i < xCoo.size(); i++) {
        distances << calcPointDistance(lineCenterX as float, lineCenterY as float, xCoo[i], yCoo[i])
    }
    minDistanceIndex = distances.indexOf(distances.min())

    // calculate length until the minDistance point
    segmentLength = 0
    for (int i = 0; i < minDistanceIndex - 1; i++) {
        segmentLength += calcPointDistance(xCoo[i], yCoo[i], xCoo[i+1], yCoo[i+1])
    }
    return Math.round((segmentLength / totalLength) * 100)
}
...
def reverseFloatList(float[] array) {
    def arrResult = array.clone()
    countForReversing = 0
    for (int i = array.length-1; i >= 0; i--) {
        //println(array[i])
        arrResult[countForReversing] = array[i]
        countForReversing++
    }
    return arrResult
}
 ...
 * @modification: removed category option (vs. version0)
 * defining base nuclei count
 * @param gdTitle = String dialog title
 * @return = Base nuclei count
 */
